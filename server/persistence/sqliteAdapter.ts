...
export class HermesSqliteAdapter {
  ...
  public saveSystemState(state: HermesSystemState): void {
    ...
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO system_state (system_id, data, updated_at)
      VALUES (?, ?, ?)
    `);
    stmt.run([state.system_id, JSON.stringify(state), new Date().toISOString()]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public saveProject(project: DigitalTwinProject): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO projects (id, name, gym_level, status, data, updated_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
    stmt.run([
      project.id,
  ...
  public insertHeartbeatRecord(record: HeartbeatRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO heartbeats (heartbeat_id, timestamp, project, data)
      VALUES (?, ?, ?, ?)
    `);
    stmt.run([record.heartbeat_id, record.timestamp, record.project, JSON.stringify(record)]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public insertTaskExecutionRecord(record: TaskExecutionRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO task_execution_records (task_id, project, trade, data, created_at)
      VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run([record.task_id, record.project, record.trade, JSON.stringify(record), record.created_at]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public insertModelRevisionRecord(record: ModelRevisionRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO model_revision_records (revision, timestamp, project, data)
      VALUES (?, ?, ?, ?)
    `);
    stmt.run([record.revision, record.timestamp, record.project, JSON.stringify(record)]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public insertInspectionAuditRecord(record: InspectionAuditRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO inspection_audit_records (inspection_id, project, inspector, final_status, data)
      VALUES (?, ?, ?, ?, ?)
    `);
    stmt.run([record.inspection_id, record.project, record.inspector, record.final_status, JSON.stringify(record)]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public insertBOMRevisionRecord(record: BOMRevisionRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO bom_revision_records (bom_revision_id, project, timestamp, data)
      VALUES (?, ?, ?, ?)
    `);
    stmt.run([record.bom_revision_id, record.project, record.timestamp, JSON.stringify(record)]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public insertDecisionLog(record: DecisionLogRecord): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO decision_log_records (id, project_id, timestamp, data)
      VALUES (?, ?, ?, ?)
    `);
    stmt.run([record.id, record.projectId, record.timestamp, JSON.stringify(record)]);
    stmt.free();
    this.saveToFile();
  }
  ...
  public saveCompetencyMatrix(matrix: CompetencyMatrix): void {
    if (!this.db) return;
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO competency_matrix (id, data, last_updated)
      VALUES (?, ?, ?)
    `);
    stmt.run(['GLOBAL_HERMES_MATRIX', JSON.stringify(matrix), matrix.lastUpdated]);
    stmt.free();
    this.saveToFile();
  }
